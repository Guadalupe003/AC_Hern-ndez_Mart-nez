...
begin
	...
	function animaCaminata2D(caminata)
	    ...
	    anim = @animate for i in 1:n
	        plot(pasos_horizontales[1:i], pasos_verticales[1:i], line=:dash, marker=:circle, aspect_ratio=:equal, legend=false)
	        xlabel!("Posición Horizontal")
	        ylabel!("Posición Vertical")
	        title!("Caminata Aleatoria en 2D")
	    end
...
# ╔═╡ 09729f78-7f8a-4715-8fc9-d639b7df0d04
begin
a = 2
b = 5
numeroAleatorio = aleatorioUniforme(a, b)
...
# Tu código va aquí :)
# Vamos a intentarlo ^^
function verificarDistribucionUniforme(c, d, x)
    # Generar una muestra de n números aleatorios
    muestra = [aleatorioUniforme(c, d) for _ in 1:x]

    # Crear un histograma de la muestra
    histograma = histogram(muestra, bins = 20, xlabel = "Valor", ylabel = "Frecuencia", color = "purple", 
...
# ╔═╡ 0d9385c6-df68-4e62-a4a4-5852466d7b66
d = 5

# ╔═╡ 995d6a62-c579-4ea7-9bdb-42e58accc0de
# Tamaño de la muestra
x = 10000

# ╔═╡ b1d56905-7282-4770-84ef-59432f6ee986
# Verificar la distribución uniforme y graficar el histograma
histograma = verificarDistribucionUniforme(c, d, x)

# ╔═╡ 07e5779f-0f01-434f-b3dc-8e1e3e39b272
display(histograma)
...
# ╔═╡ 50eb09fa-eb61-4cca-a025-30b3223c3e09
# ╠═╡ disabled = true
#=╠═╡
resultado = unPaso()
  ╠═╡ =#

# ╔═╡ a841ff20-ff2f-4d14-954c-e12be9d428f1
#=╠═╡
println(resultado)
  ╠═╡ =#
...
function variosPasos(i)
    return 2 .* rand(Bool, i) .- 1
end

# ╔═╡ 58075114-240b-4baa-93da-787bc351c1ea
begin
	i = 5
	resultado1 = variosPasos(i)
println(resultado1)
end
...
#Vamos a intentarlo ^^
function caminataAleatoria(k)
    pasos = [0]
    for _ in 1:k
        paso = unPaso(k)
        posicion_actual = pasos[end]
        nueva_posicion = posicion_actual + paso
        push!(pasos, nueva_posicion)
    end
    return pasos
end

# ╔═╡ cb532c2a-2799-4e29-a2b8-4f979886b22c
# ╠═╡ disabled = true
#=╠═╡
k = 10
  ╠═╡ =#

# ╔═╡ 303514f3-fc1f-46b9-9b0a-e4f0d9f6c61f
#=╠═╡
resultado2 = caminataAleatoria(k)
  ╠═╡ =#

# ╔═╡ f44c4a85-acb9-4da3-ba1a-67e996b3c06c
#=╠═╡
println(resultado2)
  ╠═╡ =#
...
begin
	...
	function graficaCaminata(caminata)
	    n = length(caminata)
	    pasos = 0:n-1
	    scatter(pasos, caminata, xlabel = "Número de Pasos", ylabel = "Posición", color = "yellow",
...
#generar graficas adicionales de caminatas añeatorias?
n2 = 200

# ╔═╡ 1144766d-2a54-4a1e-b3ef-931b80de76ab
caminata2= caminataAleatoria(n2)

# ╔═╡ 32bc24c1-dbb9-401a-9066-e5d37e78ecca
begin
	graficaCaminata(caminata2)
end
...
begin
	...
	begin
	function graficaCaminata!(caminatas)
		    n = size(caminatas, 2)
		    pasos = 0:n-1
		    for i in 1:size(caminatas, 1)
		        plot!(pasos, caminatas[i, :], linestyle=:dash, label="Caminata $i")
		    end
...
# Generar 5 caminatas aleatorias y graficarlas juntas
num_caminatas = 5
...
begin
	...
	function animaCaminata(caminata)
	    n = length(caminata)
	    pasos = 0:n-1
	    
	    animation = @animate for i in 1:n
	        graficaCaminata(caminata[1:i])
	    end
	    
	    gif(animation, "caminata_animada.gif", fps = 10)
	end
end


# ╔═╡ a7de77db-2c8d-4b17-ae4a-3dd57c688535
caminatagif = caminataAleatoria(100)

# ╔═╡ abff0a42-8c9e-4379-afa7-654403b08754
animaCaminata(caminatagif)
...
# Vamos a intentarlo ^^
function caminataAleatoria2D(l)
    pasos_horizontales = [0]
    pasos_verticales = [0]

    for _ in 1:l
        paso_horizontal = round(aleatorioUniforme(-1, 1))
        paso_vertical = round(aleatorioUniforme(-1, 1))

        posicion_horizontal_actual = pasos_horizontales[end]
        posicion_vertical_actual = pasos_verticales[end]

        nueva_posicion_horizontal = posicion_horizontal_actual + paso_horizontal
        nueva_posicion_vertical = posicion_vertical_actual + paso_vertical

        push!(pasos_horizontales, nueva_posicion_horizontal)
        push!(pasos_verticales, nueva_posicion_vertical)
    end

    return [pasos_horizontales, pasos_verticales]
end

# ╔═╡ ab167566-c46a-4625-a5b9-78839ffec8d4
l = 10

# ╔═╡ ebd81109-acce-4126-9c5d-415146e794ed
resultado = caminataAleatoria2D(l)

# ╔═╡ f01c93dd-537f-4701-9685-38367d1798b2
println(resultado)
...
function graficaCaminata2D(caminata)
	    ...
	    plot(pasos_horizontales, pasos_verticales, line=:dash, marker=:circle, aspect_ratio=:equal, legend=false)
	    xlabel!("Posición Horizontal")
	    ylabel!("Posición Vertical")
	    title!("Caminata Aleatoria en 2D")
	end
...
# ╔═╡ a6d1d6f0-4dfd-474d-87c0-10bd15f4df39
# Tu código (comentado) va aquí :)


function graficaCaminata2D!(p, caminata)
    pasos_horizontales = caminata[1]
    pasos_verticales = caminata[2]

    plot!(p, pasos_horizontales, pasos_verticales, line=:dash, marker=:circle)
end

# ╔═╡ 9dfee2f2-b559-4196-a4a5-72820ea9d006
p = plot(xlabel="Posición horizontal", ylabel="Posición vertical", title="Caminatas aleatorias en 2D", aspect_ratio=:equal, legent=false)

# ╔═╡ 5af465eb-8fca-43c2-bae8-43c9218150eb
for _ in 1:5
	caminata = caminataAleatoria2D(100)
	graficaCaminata2D!(p, caminata)
end
...
function graficaCaminata3D(caminata)
    ...
    plot3d(pasos_x, pasos_y, pasos_z, line=:dash, marker=:circle, legend=false)
    xlabel!("Posición X")
    ylabel!("Posición Y")
    zlabel!("Posición Z")
    title!("Caminata Aleatoria en 3D")
end

# ╔═╡ ea41df09-15d5-4402-890f-413dfb74e8d0
caminatas = zeros(num_caminatas, n+1)

# ╔═╡ 7d6df278-83fa-4ee3-b04f-aba267080b17
graficaCaminata!(caminatas)

# ╔═╡ 6b9b5846-038d-408a-abc2-750519330526
for i in 1:num_caminatas
	caminatas[1, :] = caminataAleatoria(n)
end
...
# ╔═╡ 5aaec01c-7eee-4be2-a19b-76ab2e3c4e56
# ╠═╡ disabled = true
#=╠═╡
n = 100
  ╠═╡ =#
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "85a3012689e7cfd3843b676bfdfe2cc606a10f21"
