...
# ╔═╡ 8bf5498f-9145-49a6-939f-67e9f27d46fd
using Plots, PlutoUI, Interact
...
Esto se puede resolver mediante la siguiente ecuación:
...
#Comencemos definiendo la función y los paramétros que vamos a utilizar
begin
	function newton(f,f′,x_0,n)

#Seguiremos la sugerencia, y utilizaremos un for para realizar las iteraciones
	for i in n
#Ahora implementaremos la fórmula que nos pusieron en el ejercicio anterior
	x=x_0-(f/f′)
		println("La raíz es ",x, " de ", i , " iteraciones")
	end
end
end
...
# ╔═╡ 86f0e4ba-0a67-44b9-998f-262167a9b0cb
begin

#Definimos la función newtonRecursiva
	function newtonRecursiva(f1,df,x0,n1)
#Para implementar la función recursiva utilizamos un if, que toma en cuenta todos los valores mayor o igual a cero
		if n1 >= 0
#Y nos devolverá el resultado de la fórmula anterior
	return x0-(f1/df)
	end
end
end

# ╔═╡ 57804a1d-2743-48fd-adaf-52f9540492e4
newtonRecursiva(0.125,-3.25,0.5,100)
...
#Primero vamos a escribir la condición donde epsilon debe ser mayor que cero
		if ϵ<=0
		println("Error, el valor de epsilon es menor o igual que cero")
		end
#Ahora bien, si pasa este filtro va a poder realizar el método newton 
...
# ╔═╡ 5114c096-2d06-42c7-a5e9-1a1efe8ef007
newtonDistancia(0.125,-3.25,0.5,0.1)
...
#Utilizamos el hint que el ejercicico nos da, usando la sintaxis nombre(parámetro1, parámetro2,) = expresión
derivadaNumérica(F1,X,h)= (F1(X+h) - F1(X))/h

# ╔═╡ 731def47-b524-44e2-beb7-ed88acd6261f
derivadaNumérica(cos, 0, 0.1)
...
#Comencemos definiendo la función y sus parametros
newtonDN(F_1,X_0,H)=X_0 - F_1(X_0)/derivadaNumérica(F_1, X_0, H)

# ╔═╡ 11034496-a28a-4ea7-90f6-47e22b00e299
newtonDN(cos, 0, 0.1)
...
# ╔═╡ 4cec0bb7-6270-401c-8f84-c8a5647508dc
begin
# La función que deseamos encontrar sus raíces.
f2(x) = cos(x)

# La derivada de la función.
df(x) = -sin(x)

# Implementamos la ecuación del método de Newton, donde implementaremos lo de ejercicios anteriores
	function newton(f2, df, x0, eps=1e-7, max_iter=10)
    x = x0
    for _ in 1:max_iter
        x_00 = x
        fx = f2(x)
        dfx = df(x)
        #A continuación, se pondrán las condiciones que debe cumplir nuestra función 
        # Debe evitarse la división por cero.
        if abs(dfx) < eps
            return x
        end

		#Escribimos la ecuación del método de newton
        x = x_00 - fx/dfx
        # Aplicamos lo de ejercicios anteriores, de las distancias
        if abs(x - x_00) < eps
            return x
        end
    end
...
# Graficar función y raíces.
	function graficar(f2, raices, limites=(-2*pi, 2*pi))
    x = range(limites[1], limites[2], length = 1000)
    y = f2.(x)

    plot(x, y, label="cos(x)", linewidth=3)
    scatter!(raices, f2.(raices), label="Raices", color="green")
	end


raices = []

	for x0 in -2*pi:pi/2:2*pi, h in 1000:1000:1000000
	#Llamamos nuestra función que definimos al inicio del programa
    raiz = newton(f2, df, x0)
    push!(raices, raiz)
	end
...
# ╔═╡ 134a2581-b0bc-4cac-a5a4-88060500f32b
newton(0.125,-3.25,0.5,100)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "8645f4f9a15883bc985ebf735a7b46f6da319df3"
