...
# ╔═╡ 9ce7f7c8-2623-4815-be0a-790b225e9e50
using Plots
...
begin
	...
	function animaCaminata2D(caminata)
	    pasos_horizontales = caminata[1]
	    pasos_verticales = caminata[2]
	
	    n = length(pasos_horizontales)
	
	    anim = @animate for i in 1:n
	        plot(pasos_horizontales[1:i], pasos_verticales[1:i], line=:dash, marker=:circle, aspect_ratio=:equal, legend=false, xlabel = "Posiución horizontal", ylabel ="Posición vertical", title="Caminata Aleatoria en 2D")
	    end
...
# ╔═╡ 710df1bb-d7ed-4919-944a-72f97471ce04
md"Carol Michelle Salgado Gómez"
...
# ╔═╡ 3c0eef33-4301-479e-99fb-98ecbc88def8
begin
#Definimos la función que se nos pide junto con los parametros a y b 
	function aleatorioUniforme(a, b)
#En general, podemos generar números aleatorios con la fórmula a+(b-a)*rand(N)
    	return a + (b - a) * rand()
	end
end

# ╔═╡ 8f50b300-0015-4de4-ac54-61565c2bb0a0
aleatorioUniforme(2, 5)

# ╔═╡ 737eb77d-ce3f-4eac-9f38-5cbae8bd2636
md"**Ejercicio** Verifica con un histograma con una función `aleatorioUniforme` realmente cumpla la propiedad desada. (Pista: ¿Cómo debería verse el histograma?)"

# ╔═╡ bd81b8af-5c1f-47ac-aa19-ec98cbc704b1
begin
#Creamos la función así como sus parametros que vamos a utilizar aleatoriamente y hasta donde va a llegar a implementarse el for
	function HistogramaDU(x, y, z)

    h_DU = [aleatorioUniforme(x, y) for _ in 1:z]

#Utilizamos la función histogram para graficarlo
    grafica = histogram(h_DU,title = "Histograma: Distribución Uniforme", 
	xlabel = "x", ylabel = "y", color = "lightblue",
	bins = 10)
    return grafica
end
end

# ╔═╡ db319272-7adb-4d8d-bac1-f966d0027c1e
HistogramaDU(2, 5, 10)
...
# ╔═╡ 27785540-d91d-4ebc-938f-313e5e0c9316
begin
#El ejercicio nos pide una función que no tome argumentos
	function unPaso()
#Utilizamos la función rand(bool) para generar un valor aleatorio que devuelva solo 1 y-1
   		return 2 * rand(Bool) -1 
	end
end

# ╔═╡ 37799189-af9f-4958-acae-6771a4fa5df7
unPaso()
...
# ╔═╡ 9786b509-43ed-47c2-8543-c9f6dd313ccc
begin
#Definimos la función varios pasos y con el parametro n
	function variosPasos(n)
#Utilizamos ahora la misma fórmula que en la celda anterior
    	return 2 .* rand(Bool, n) .- 1
	end
end

# ╔═╡ 58075114-240b-4baa-93da-787bc351c1ea
variosPasos(10)
...
# ╔═╡ f56bd374-f3cb-46a6-82ff-01b0aa7cf01c
begin
#Definimos nuestra función caminataAleatoria con un número entero positivo
	function caminataAleatoria(n1) 
#Definimos la posición_a igual a cero para que inicie en 0 como se nos pide
    	posicion_a=0
		caminante=[posicion_a]
#Mi arreglo va a ir de 1 a n1
    for _ in 1:n1
#Utilizamos la función un paso que anteriormente habiamos definido
        pasos = unPaso()
        posicion_a += pasos
#Utilizamos push para agregar al arreglo un valor más
        push!(caminante, posicion_a)
    end
    return caminante
end
end

# ╔═╡ 785cac69-1ceb-4db5-b1f5-1a79ce20bf0f
caminataAleatoria(9)
...
# ╔═╡ ea0ca2a4-2d85-4dd1-b6f7-6bf1c691792a
begin
#Definimos la función que se nos pide con un parámetro n2 y tomamos como parametro un arreglo 
	function graficaCaminata(cam_al)
	    n = length(cam_al)
#La función pasos que definiremos a continuación, va a contar los pasos que se van generando
	    pasos = 1:n+1
#Como el ejercicio nos dice que posición vs número de pasos son x y y
	scatter(pasos, title = "Caminata Aleatoria", cam_al, xlabel = "posición ", ylabel = "Número de pasos")
	end
	
end

# ╔═╡ c8cba0c2-19ff-4b48-88b8-6b234f7eb21a
begin
#Seguimos la sugerencia que nos da el ejercicio
	n2 = 100
#Aplicamos la función cumsum para que sea en una dimención
cam_al= cumsum(rand([-5,9 ], n2))

# Genera la gráfica de la caminata aleatoria
graficaCaminata(cam_al)
end
...
# ╔═╡ eeb54161-3bce-4b01-9575-e0d0ccbc7c91
begin
#Comencemos definiendo la función graficaCaminata
	function graficaCaminata!(cams_al)
		    n = length(cams_al[1])
		    pasos = 0:n-1

		plot(pasos, cams_al,  xlabel = "posición ", ylabel = "Número de pasos")
		   
	end
end

# ╔═╡ 641fad20-aa89-4058-b05f-06178cde2a0d
begin
#Seguimos la sugerencia que nos da el ejercicio
	n_2 = 100
#Aplicamos un for que indique que va del uno al 5
cams_al= [cumsum(rand([-1, 1], n_2)) for _ in 1:5]

# Genera la gráfica de la caminata aleatoria
graficaCaminata!(cams_al)
end
...
begin
	function animaCaminata(camin_al)
	    n = length(camin_al)
	    pasos = 1:n
#Como nos piden animar utilizaremos la función animate
	    animation = @animate for j in 1:n
	        plot(pasos[1:j], camin_al[1:j],xlabel = "posición ", ylabel = "Número de pasos")
	    end
...
# ╔═╡ 3ccfd8bc-e6ec-48dd-a450-e3708aa4f2e0
begin
#Seguimos la sugerencia que nos da el ejercicio
	n_1 = 100
#Aplicamos un for que indique que va del uno al 5
camin_al= cumsum(rand([-10, 10], n_1))

# Genera la gráfica de la caminata aleatoria
graficaCaminata(cam_al)

animation = animaCaminata(cam_al)

# Muestra la animación en el notebook
gif(animation, "caminata.gif", fps = 10)
end
...
# ╔═╡ 95d8ec93-f95d-4f3f-aecf-d5e02195f7aa
begin
function caminataAleatoria2D(n_3)
    pasos_h = [0]
    pasos_v = [0]

    for _ in 1:n_3
#Hacemos llamado a la función aleatorioUniforme 
        paso_h = round(aleatorioUniforme(-1, 1))
        paso_v = round(aleatorioUniforme(-1, 1))
#Indicamos la posición actual y le asignamos los pasos 
        posicion_h_a = pasos_h[end]
        posicion_v_a = pasos_v[end]
#Indicamos la n ueva posición y le asignamos los pasos 
        nueva_posicion_horizontal = posicion_h_a + paso_h
        nueva_posicion_vertical = posicion_v_a + paso_v
#Utilizamos push para agregar al arreglo un valor más
        push!(pasos_h, nueva_posicion_horizontal)
        push!(pasos_v, nueva_posicion_vertical)
    end

    return [pasos_h, pasos_v]
end
end

# ╔═╡ ab167566-c46a-4625-a5b9-78839ffec8d4
begin
	n_3 = 10
	resultado = caminataAleatoria2D(n_3)
	println(resultado)
end
...
function graficaCaminata2D(caminata)
	    pasos_horizontales = caminata[1]
	    pasos_verticales = caminata[2]
#Comenzamos a graficar la función
	    plot(pasos_horizontales, pasos_verticales, line=:dash, marker=:circle,  		legend=false, xlabel="Posición Horizontal", ylabel="Posición 			Vertical",title="Caminata Aleatoria en 2D")
	end
	
end

# ╔═╡ 3e7c5d91-2640-4f40-8090-405000eb59c5
begin
	n3= 10;
	Caminata = caminataAleatoria2D(n3)
	graficaCaminata2D(Caminata)
end
...
# ╔═╡ 9337f2a6-39fc-4315-aa32-a413f7266e29
begin
function graficaCaminata2D!(cam)
#Utilizamos un for para que va de c a cam
    for c in cam
        plot!(c[1, :], c[2, :], xlabel = "x ", ylabel = 		"y")
    end
end
end
...
# ╔═╡ 8e0f1e61-3a09-4d0a-9d5e-3941b2d72949
begin
	caminata = caminataAleatoria2D(100)
	anim = animaCaminata2D(caminata)
end

# ╔═╡ 65a0036e-763f-431b-8e61-ee193815b29f
gif(anim, "caminata.gif")
...
# ╔═╡ 282b533a-2007-4c9c-a9f7-fc4ebea85ffb
begin

function caminataAleatoria3D(n)
    pasos_x = [0.0]
    pasos_y = [0.0]
    pasos_z = [0.0]
    ...
    for _ in 1:n
        ...
        posicion_x_actual = pasos_x[end]
        posicion_y_actual = pasos_y[end]
        posicion_z_actual = pasos_z[end]

        nueva_posicion_x = posicion_x_actual + paso_x
        nueva_posicion_y = posicion_y_actual + paso_y
        nueva_posicion_z = posicion_z_actual + paso_z

        push!(pasos_x, nueva_posicion_x)
        push!(pasos_y, nueva_posicion_y)
        push!(pasos_z, nueva_posicion_z)
    end

    return [pasos_x, pasos_y, pasos_z]
end
end

# ╔═╡ 510f7147-5a2b-4ecc-866f-166ffdd326c8
begin
function graficaCaminata3D(caminata)
#Graficamos la gráfica de x, y y z
    pasos_x = caminata[1]
    pasos_y = caminata[2]
    pasos_z = caminata[3]
#Utiloizamos la función plot para graficar
    plot3d(pasos_x, pasos_y, pasos_z, line=:dash, marker=:circle, 			legend=false,xlabel="Posición X", ylabel="Posición Y",zlabel="Posición 	Z",title="Camina Aleatoria en 3D")
   
end
end

# ╔═╡ 508b70b4-f8ef-40dc-b683-986985e5121f
n = 100

# ╔═╡ 213112db-7670-46ac-be4a-4f5b338b4d16
Caminata1 = caminataAleatoria3D(n)

# ╔═╡ 9cf1ea65-d3fb-40c5-a581-72ba1ec4411e
graficaCaminata3D(Caminata1)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "20372e83cba4cbe0f6d92025898044208f7edd76"
