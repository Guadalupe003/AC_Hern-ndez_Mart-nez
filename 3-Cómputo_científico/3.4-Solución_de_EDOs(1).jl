...
#Escribimos el nombre de la función y le asignamos lo que debe realizar
		tamañoDePaso(t0,tf,N)= (tf-t0)/N

# ╔═╡ 5ad00760-0e11-4695-ba1d-0e17b6529108
tamañoDePaso(1,3,9)
...
begin
#Definimos la función con la que vamos a trabajar, que en este caso es arregloUniforme
	function arregloUniforme(t0,tf,h)                     

#Ahora pondremos la forma i donde pareciera actuar como un vector que multiplica a h y después suma la posición inicial 
		array=[i*h+t0 for i ∈ 0:100]                                                          
	return array
	end
end


# ╔═╡ cd81a2e1-8e55-4878-9ede-52fcc6040afe
arregloUniforme(-10,10,0.2)
...
_Probablemete, tendríamos que utilizar la ecuación que se utiliza para aproximar con el método de euler, la cual es la siguiente: 

$$x(t_1) \approx Xi + hg(t_0,x(t_0)).$$
...
# ╔═╡ 2762841a-6a88-4578-bb59-2a77d07d97d2
deslizador3 = @bind z1 Slider(-10:0.1:10, default = 1)

# ╔═╡ 82f05ae1-5d79-4c1b-83b6-66c910349a5b
paso_euler(0,1,g,1)
...
# ╔═╡ 9d23518c-9c7c-486c-b2c2-7d605a2bc040
begin
	# Definir la función g en función de x y t
function g(x, p)
    return [3x[1] - 2x[2], 4x[1] - x[2]]
end
...
function euler(g, xt0, t)
    N = length(t)
    h = t[2] - t[1]
    xt = zeros(N, length(xt0))
    xt[1, :] = xt0

    for i in 1:N-1
        xt[i+1, :] = xt[i, :] + h * g(xt[i, :], t[i])
    end
...
# Generar arreglo uniforme de tiempos
t = range(t0, tf, length=N)

# Obtener la solución numérica mediante el método de Euler
approximation = euler(g, xt0, t)

# Definir la solución analítica
analytical_solution = zeros(N, 2)
for i in 1:N
    analytical_solution[i, 1] = cos(t[i])
    analytical_solution[i, 2] = sin(t[i])
end

# Graficar la solución numérica y analítica
plot(p, approximation[:, 1], label="Aproximación numérica", xlabel="t", ylabel="x", title="Solución numérica vs. Solución analítica")
plot!(p, analytical_solution[:, 1], label="Solución analítica")
end

# ╔═╡ 254c62b5-8d32-49eb-ae91-0be2ebc3bc2e
begin
	function g(ti,xti)
		return (1)
	end
#Ahora definimos la función paso euler, y los parametros
	function paso_euler(ti,xti,g,h)
#Utilizamos la ecuación dada anteriormente
		paso_euler=xti+h*g(ti,xti)
...
begin
#Creamos la función método_euler, con su parametro respectivo
	function método_euler(g,xt0,t)
		return(1*t)*2
	end
		g(t)= (1*t)* 2
#La condición inicial que imponemos es:
		xt0=5
		t= range(-10,10,3)
		println("G: ", método_euler(g, xt0, t))
end

# ╔═╡ 7c8c2188-3173-4202-8ba4-83554831d940
begin
#Definamos la función euler con sus parametros correspondientes
	function euler(g, xt0 ,t)
#La función lenght solo nos indica el tamaño del arreglo y va a ser respecto de t
		R= length(t)
#Lo siguiente, va a ser para almacenar el arreglo
		xt= zeros(R)
		xt0 = xt[1]
	for i in 2:R
#Podemos utilizar una de las ecuaciones para aproximar mediante el método de euler
		xt[i]=xt[i-1]+ (t[i]- t[i-1])* g(t[i-1], xt[i-1]) 
	end
		return xt
	end
	aprox= euler(g, xt0 ,t)
println("La aproximación por el método de euler, es: ", aprox)
end
